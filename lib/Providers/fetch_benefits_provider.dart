  ...
  Future<BenefitsResponse?> fetchBenefits() async {
    try {
      ...
      if (response.statusCode == null || response.body == null) {
        failure('Error in fetchBenefits', 'Server Failed To Respond');
        return null;
      }
      if (response.statusCode == 200) {
        if (response.body['error']['code'] == 0) {
          ...
          failure('Error in fetchBenefits', response.body['error']['message']);
          return null;
        }
      } else {
        failure('Error in fetchBenefits', response.body.toString());
        return null;
      }
    } catch (e) {
      failure('Error in fetchBenefits', e.toString());
      return null;
    }
  }
}
