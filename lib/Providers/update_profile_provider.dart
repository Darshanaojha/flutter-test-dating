    ...
    try {
      ...
      if (token != null && token.isNotEmpty) {
        ...
        if (response.statusCode == null || response.body == null) {
          failure('Error in updateProfile', 'Server Failed To Respond');
          return null;
        }
        ...
        if (response.statusCode == 200) {
          if (response.body['error']['code'] == 0) {
            ...
            failure('Error in updateProfile', response.body['error']['message']);
            return null;
          }
        } else {
          failure('Error in updateProfile', response.body.toString());
          return null;
        }
      } else {
        failure('Error in updateProfile', 'Token not found');
        return null;
      }
    } catch (e) {
      failure('Error in updateProfile', e.toString());
      return null;
    }
  }
}
