  ...
  Future<SafetyGuidelinesResponse?> fetchAllSafetyGuidelines() async {
    try {
      ...
      if (response.statusCode == null || response.body == null) {
        failure('Error in fetchAllSafetyGuidelines', 'Server Failed To Respond');
        return null;
      }
      if (response.statusCode == 200) {
        if (response.body['error']['code'] == 0) {
          ...
          failure('Error in fetchAllSafetyGuidelines', response.body['error']['message']);
          return null;
        }
      } else {
        failure(
      ...
      failure('Error in fetchAllSafetyGuidelines', e.toString());
      return null;
    }
  }
}
